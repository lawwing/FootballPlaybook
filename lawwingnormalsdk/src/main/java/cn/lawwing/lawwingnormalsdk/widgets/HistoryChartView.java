package cn.lawwing.lawwingnormalsdk.widgets;
...
import cn.lawwing.lawwingnormalsdk.R;
import cn.lawwing.lawwingnormalsdk.utils.DisplayUtils;

/**
 * 历史记录查询图表 基于pathMeasure+DashPathEffect+属性动画实现 绘制时采取双缓冲绘制
 *
 * @author zyw
 * @creation 2017-03-06
 */
public class HistoryChartView extends View
{
    ...
    private float[] powerOnTimeDataArray = { 100, 100, 100, 100, 100, 100,
            100 };
            ...
            int defStyleAttr)
    {
        ...
        // Logger.e("HistoryChartView(Context context, AttributeSet attrs, int
        // defStyleAttr)");
        
        ta = context.obtainStyledAttributes(attrs,
                R.styleable.HistoryChartView);
    ...
    private void initXData(String[] tempData)
    {
        switch (mMode)
        {
            case DAY_MODE:
                ...
                setXUnitText(
                        getResources().getString(R.string.history_x_unit_hour));
                break;
            case WEEK_MODE:
                xScaleForData = X_SCALE_FOR_DATA_WEEK;
                setXUnitText(
                        getResources().getString(R.string.history_x_unit_day));
                break;
            case MONTH_MODE:
    ...
    {
        float MarginTopPx = ta
                .getDimension(R.styleable.HistoryChartView_margin_top, 50);
        float MarginBottomPx = ta
                .getDimension(R.styleable.HistoryChartView_margin_bottom, 50);
        float MarginLeftPx = ta
                .getDimension(R.styleable.HistoryChartView_margin_left, 50);
        float MarginRightPx = ta
                .getDimension(R.styleable.HistoryChartView_margin_right, 50);
        ...
        float yUnitSizePx = ta.getDimension(
                R.styleable.HistoryChartView_y_unit_text_size,
                30);
        ...
        float lineStrokeWidthPx = ta.getDimension(
                R.styleable.HistoryChartView_line_stroke_width,
                5);
        float dataStrokeWidthPx = ta.getDimension(
                R.styleable.HistoryChartView_data_stroke_width,
                5);
        float circleRadiusPx = ta
                .getDimension(R.styleable.HistoryChartView_circle_radius, 6);
    ...
    private void drawGreyXLine(Canvas canvas, Paint p)
    {
        ...
        // 纵向
        for (int i = yScaleForData; (yLength
                - i * Yscale) >= 0; i += yScaleForData)
        {
            float startY = Ypoint - i * Yscale;
            canvas.drawLine(startX
                    - MarginLeft / 4, startY, xLength + MarginLeft, startY, p);
        }
    }
    ...
    private void drawData(Canvas canvas, float[] data, int dataColor)
    {
        ...
        for (int i = 0; i < Xlabel.length; i++)
        {
            ...
            if (xLableInt % xScaleForData == 0)
            {
                ...
                canvas.drawText(this.Xlabel[i],
                        startX - mXlabelSize / 3,
                        Ypoint + mXlabelSize * 3 / 2,
                        p);
            }
            p.setColor(dataColor);
            canvas.drawCircle(startX,
                    getDataY(data[i], Ylabel),
                    circleRadius,
                    p);
        }
        ...
        {
            p.setColor(lineColor);
            canvas.drawText(this.Ylabel[i],
                    MarginLeft / 4,
                    getDataY(Float.valueOf(Ylabel[i]), Ylabel)
                            + mYLabelSize / 3,
                    p);
            canvas.drawText(this.Ylabel2[i],
                    this.getWidth() - MarginLeft,
                    getDataY(Float.valueOf(Ylabel2[i]), Ylabel2)
                            + mYLabelSize / 3,
                    p);
        }
    }
    ...
    private void drawRect(Canvas canvas, float[] data, int dataColor)
    {
        ...
        for (int i = 0; i < Xlabel.length; i++)
        {
            ...
            if (i != 0)
            {
                left = Xpoint + (i - 1) * Xscale + xFirstPointOffset
                        + Xscale / 6;
    ...
    private void drawY1Unit(Canvas canvas, Paint p)
    {
        ...
        canvas.drawText(mY1UnitText,
                MarginLeft / 2 - textWidth / 2,
                getDataY(maxYLabelValue, Ylabel) - mYLabelSize
                        - mYLabelSize / 5,
                p);
    }
    ...
    private void drawY2Unit(Canvas canvas, Paint p)
    {
        ...
        canvas.drawText(mY2UnitText,
                this.getWidth() - MarginRight / 2 - textWidth * 3 / 4,
                getDataY(maxYLabel2Value, Ylabel2) - mYLabelSize
                        - mYLabelSize / 5,
                p);
    }
    ...
    private void drawXUnit(Canvas canvas, Paint p)
    {
        ...
        canvas.drawText(mXUnitText,
                this.getWidth() / 2 - textWidth / 2,
                Ypoint + mXlabelSize * 3 + mXlabelSize / 5,
                p);
    }
            ...
            int bottom)
    {
        ...
        // 创建一个与该View相同大小的缓冲区
        mCacheBitmap = Bitmap.createBitmap(this.getWidth(),
                this.getHeight(),
                ...
                .addUpdateListener(new ValueAnimator.AnimatorUpdateListener()
                {
                    ...
                    public void onAnimationUpdate(ValueAnimator animation)
                    {
                        ...
                        mtargetTempEffect = new DashPathEffect(
                                new float[] { targetTempLength,
                                        targetTempLength },
                                fraction * targetTempLength);
                        targetTempPaint.setPathEffect(mtargetTempEffect);
                        // 更新mRoomTempEffect
                        mRoomTempEffect = new DashPathEffect(
                                new float[] { roomTempLength, roomTempLength },
                                fraction * roomTempLength);
                        roomTempPaint.setPathEffect(mRoomTempEffect);
                        // 更新rect绘制fraction进度
                        mRectFration = 1 - fraction;// fraction是1->0
                                                    // 我们需要的柱形图绘制比例是0->1
                        postInvalidate();
                    }
                });

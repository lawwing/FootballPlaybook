...
import static cn.lawwing.lawwingnormalsdk.utils.LogUtils.e;
...
public class NetworkConnectionUtils
{
     ...
     * @return
     */
    public static boolean connectToSocketWifi(WifiManager manager,
            String wifiSSID)
    {
        ...
        wifiConfiguration.wepKeys[0] = "\"" + "\""; // 小米手机MIUI7/华为EMUI4.1
                                                    // 需要webKey
        ...
        else
        {
            ...
            // wifiConfiguration.wepKeys[0] = "\"" + "\"";//去掉webKey
            // //小米手机MIUI8不能有webKey
     ...
     * @param password 密码
     */
    public static void connectWifiSSID(Context context, WifiManager manager,
            String ssid, String password)
    {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
        {
            ...
            new WifiAutoConnectManager(manager).connect(ssid,
                    password,
                    WifiAutoConnectManager.getCipherType(context, ssid));
        }
        else
        {
            int networkId = manager
                    .addNetwork(getWifiConfiguration(manager, ssid, password));
            if (networkId != -1)
            {
                manager.enableNetwork(networkId, true);
            }

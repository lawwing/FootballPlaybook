...
public class MovingImageView extends ImageView
{
    ...
    public MovingImageView(Context context, AttributeSet attrs, int defStyle)
    {
        ...
        TypedArray ta = context.getTheme().obtainStyledAttributes(attrs,
                R.styleable.MovingImageView,
                defStyle,
                0);
        
        try
        {
            maxRelativeSize = ta.getFloat(
                    R.styleable.MovingImageView_miv_max_relative_size,
                    3.0f);
            minRelativeOffset = ta.getFloat(
                    R.styleable.MovingImageView_miv_min_relative_offset,
                    0.2f);
            mSpeed = ta.getInt(R.styleable.MovingImageView_miv_speed, 50);
            mRepetitions = ta
                    .getInt(R.styleable.MovingImageView_miv_repetitions, -1);
            startDelay = ta.getInt(R.styleable.MovingImageView_miv_start_delay,
                    0);
            loadOnCreate = ta.getBoolean(
                    R.styleable.MovingImageView_miv_load_on_create,
                    true);
        }
        finally
        {
    ...
    private void updateOffsets()
    {
        ...
        offsetWidth = (imageWidth - canvasWidth - minSizeX) > 0
                ? imageWidth - canvasWidth
                : 0;
        offsetHeight = (imageHeight - canvasHeight - minSizeY) > 0
    ...
    private float calculateTypeAndScale()
    {
        ...
        float scale = 1f;
        float scaleByImage = Math.max(imageWidth / canvasWidth,
                imageHeight / canvasHeight);
         ...
         // 画布宽度/图片宽度
            ...
            else if (sW < sH)
            {
                ...
                matrix.setTranslate(0,
                        (canvasHeight - imageHeight * scale) / 2f);
        ...
        {// 图片太大，根据最大比值设定图片缩放值
            ...
            if (imageWidth * scale < canvasWidth
                    || imageHeight * scale < canvasHeight)
            {
                scale = Math.max(canvasWidth / imageWidth,
